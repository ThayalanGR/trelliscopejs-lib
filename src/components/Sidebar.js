...
import { displayLoadedSelector, configSelector } from '../selectors';
import {
...
const Sidebar = ({
  classes, styles, active, displayLoaded, cfg
}) => {
  if (active === '') {
    return (
      <div
  ...
  return (
    <div className={classes.sidebarContainer} style={{ ...styles.sidebarContainer, top: cfg.header.enabled ? uiConsts.header.height : 0 }}>
      <div className={classes.header} style={styles.header}>{active}</div>
      {content}
    </div>
  );
};
...
const stateSelector = createSelector(
  ...
  displayLoadedSelector, filterColSplitSelector, configSelector,
  (ch, active, displayLoaded, colSplit, cfg) => ({
    styles: {
      sidebarContainer: {
