...
import { filterViewSelector, curDisplayInfoSelector, configSelector, sortSelector, filterSelector } from '.';
import uiConsts from '../assets/styles/uiConsts';
import { sideBarVisiblityHandler, footerVisiblityHandler } from '../actions';
...
export const contentWidthSelector = createSelector(
  windowWidthSelector, sidebarActiveSelector, filterColSplitSelector, configSelector,
  (ww, active, colSplit, cfg) => {
    const sw = uiConsts.sidebar.width * (1 + (active === SB_PANEL_FILTER && colSplit && colSplit.cutoff !== null));
    let op = ww - uiConsts.sideButtons.width - (active === '' ? 0 : (sw + 1));;
    op = sideBarVisiblityHandler(cfg) ? op : ww;
    return op;
  }
);

export const contentHeightSelector = createSelector(
  windowHeightSelector, configSelector, sortSelector, filterSelector,
  (wh, cfg, sort, filter) => {
    const filterKeys = Object.keys(filter.state);
    const isFooterVisible = footerVisiblityHandler(cfg, sort, filterKeys);
    const isHeaderVisible = cfg.header.enabled;
    let op = wh;
    if (isHeaderVisible)
      op -= uiConsts.header.height;
    if (isFooterVisible)
      op -= uiConsts.footer.height
    return op;
  }
);
